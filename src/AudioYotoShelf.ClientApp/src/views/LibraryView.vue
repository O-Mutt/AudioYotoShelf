...
<template>
  <div class="space-y-6">
      ...
      <div class="flex items-center space-x-3">
        ...
        <div class="flex bg-gray-100 rounded-lg p-1">
          <button
            ...
            :class="
              libraryStore.viewMode === 'books'
                ? 'bg-white shadow-sm text-yoto-blue'
                : 'text-gray-500'
            "
          >
            Books
    ...
    <div v-if="libraryStore.viewMode === 'books'" class="flex items-center gap-3 flex-wrap">
      ...
      <div class="flex items-center gap-2">
        ...
        <button
          ...
          :title="
            libraryStore.sortDesc
              ? 'Descending (click for ascending)'
              : 'Ascending (click for descending)'
          "
        >
          <svg
            v-if="!libraryStore.sortDesc"
            xmlns="http://www.w3.org/2000/svg"
            class="h-4 w-4"
            viewBox="0 0 20 20"
            fill="currentColor"
    ...
    <div
      v-else-if="libraryStore.viewMode === 'books' && libraryStore.hasNoResults"
      ...
      <p class="mt-2 text-sm text-gray-400">
        {{
          libraryStore.searchQuery
            ...
            : 'Add some audiobooks to Audiobookshelf to get started.'
        }}
      </p>
      <button
        v-if="libraryStore.searchQuery"
    ...
    <!-- Books Grid -->
    <div
      v-else-if="libraryStore.viewMode === 'books'"
    ...
    >
      ...
      >
        ...
        <div
          ...
          :class="
            isSelected(item.id) ? 'bg-yoto-blue border-yoto-blue' : 'bg-white/80 border-gray-300'
          "
        >
          <svg
    ...
    <!-- Series Grid -->
    <div
      v-else-if="libraryStore.viewMode === 'series'"
    ...
    <div
      v-if="
        !libraryStore.isLoading &&
        libraryStore.viewMode === 'books' &&
        libraryStore.totalItems > libraryStore.pageSize
      "
    ...
    >
      ...
      <button
        @click="libraryStore.loadItems(libraryStore.currentPage + 1)"
        :disabled="
          (libraryStore.currentPage + 1) * libraryStore.pageSize >= libraryStore.totalItems
        "
        class="btn-secondary text-sm"
      >
        Next
    ...
    <div
      v-if="
        !libraryStore.isLoading &&
        libraryStore.viewMode === 'series' &&
        libraryStore.totalSeries > libraryStore.seriesPageSize
      "
      class="flex justify-center items-center space-x-2 pt-4"
    >
      <button
      ...
      <button
        @click="libraryStore.loadSeries(libraryStore.seriesPage + 1)"
        :disabled="
          (libraryStore.seriesPage + 1) * libraryStore.seriesPageSize >= libraryStore.totalSeries
        "
        class="btn-secondary text-sm"
      >
        Next
    ...
    >
      ...
      >
        <span class="text-sm font-medium text-gray-700"> {{ selectionCount }} selected </span>
        <button
